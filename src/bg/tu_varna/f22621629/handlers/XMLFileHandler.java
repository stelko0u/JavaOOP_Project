...
public class XMLFileHandler {
  ...
  public void open(String filePath) throws FileExceptionHandler {
    ...
    try (BufferedReader reader = new BufferedReader(new FileReader(filePath))) {
      ...
      throw new FileExceptionHandler("Error reading the file!", e);
    }
  }

  private void validateXmlFile(String filePath) throws FileExceptionHandler {
    if (!filePath.endsWith(".xml")) {
      throw new FileExceptionHandler("Only XML files can be opened!");
    }
  }

  private void createFileIfNotExist(File file) throws FileExceptionHandler {
    if (!file.exists()) {
      try {
        if (!file.createNewFile()) {
          throw new FileExceptionHandler("Failed to create a new file!");
        } else {
          writeInitialContentToFile(file.getPath());
        }
      } catch (IOException e) {
  ...
  public void saveSession(int sessionId, String fileName) {
    Session session = sessions.computeIfAbsent(sessionId, k -> new Session(sessionId));
  ...
  public void printSessions() {
    ...
    for (Session session : sessions.values()) {
      ...
      for (String files : session.getFileNames()) {
